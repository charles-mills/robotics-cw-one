...
from grovepi import \
    statisticalNoiseReduction  # importing function which is meant for removing outliers from a given set
    ...
    def run(self):
        ...
        while not self.event_stopper.is_set():
            ...
            if len(values) > 0:
                ...
                humidity = numpy.mean(
                    statisticalNoiseReduction([x["hum"] for x in values], self.filtering_aggresiveness))
